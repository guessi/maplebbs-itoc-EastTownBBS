...
#ifdef MY_FAVORITE
#define MSG_ZONE_SWITCH	"快速切換：(A)精華 (B)文章 (C)看板 (F)最愛 (M)信件 (U)使用者 (W)水球 (Q)離站："
#else
#define MSG_ZONE_SWITCH	"快速切換：(A)精華 (B)文章 (C)看板 (M)信件 (U)使用者 (W)水球 (Q)離站："
#endif
  ...
  XO *xo;
{
  ...
  sprintf(ftmp, "轉寄給 [%s] 嗎 (Y/N/Q)？ [Y] ", rcpt);
  sto = vans(ftmp);
  ...
  do
  {
	...
	if ((cc = bsmtp(fpath, title, rcpt, 0)) < 0)
      ...
      /* guessi.100517 若轉寄到信箱(站內/站外) 留下紀錄 (僅看板操作時) */
      if (*fpath == 'b')
      {
        if (fp = fopen(fpath, "a"))
        {
	  ...
	  time(&now);
          ptime = localtime(&now);
          /* 格式自訂 */
          fprintf(fp, "\033[1;30m轉 %s:轉寄到%s信箱%-*s%02d/%02d %02d:%02d\033[m\n",
                      cuser.userid, method ? "站內" : "私人", 49 - strlen(cuser.userid), "",
  ...
  int op;
{
  ...
  for (;;)
  {
    ...
    if (op & RS_UNREAD)
    {
#define UNREAD_FUNC()   (op & RS_BOARD ? brh_unread(BMAX(fhdr->chrono, fhdr->stamp)) : !(fhdr->xmode & MAIL_READ))
      if (op & RS_FIRST)	/* 首篇未讀 */
      {
	if (UNREAD_FUNC())
	{
...
XZ xz[] =
{
  ...
  {NULL, NULL, M_XMODE, FEETER_TOEAT}           /* XZ_TOEAT */
};
  ...
  int cmd;
{
  ...
  for (;;)
  {
	...
	if (pos < 0)
	{
	  /* pos = (zone == XZ_POST) ? 0 : num; */
	  /* itoc.000304: 閱讀到第一篇按 KEY_UP 或 KEY_PGUP 不會翻到最後 */
	...
	/* itoc.020124: 要避免如果在倒數第二頁按 KEY_PGDN，
