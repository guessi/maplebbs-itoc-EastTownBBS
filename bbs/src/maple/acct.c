...
find_rform(userno, userid, address, phone, career)
  ...
  char *userid;
  char *address, *phone, *career;    /* 回傳 address/phone/career */
{
  RFORM *rform;
  int fd, rc;
  ...
  int adm;            /* 0: user info  1: admin  2: reg-form */
{
  ...
  if (adm != 2)
    prints("  \033[32m英文代號：\033[37m%-35s", uid);

  /* guessi.100330 若為管理模式 才顯示用戶編號 */
  if (adm == 1)
    prints("\033[32m用戶編號：\033[37m%d", u->userno);
  prints("\n");

  prints("  \033[32m我的暱稱：\033[37m%-35s\033[32m擁有銀幣：\033[37m%d\n", u->username, u->money);
  prints("  \033[32m真實姓名：\033[37m%-35s\033[32m擁有金幣：\033[37m%d\n", u->realname, u->gold);
  /* guessi. 060414 雖然限制只能填寫男女 但先前資料可能不為 (1)男 (2)女 */
  prints("  \033[32m出生日期：\033[37m民國 %02d 年 %02d 月 %02d 日             \033[32m我的性別：\033[37m%.2s\n", u->year, u->month, u->day, (u->sex == 1) ? "♂" : ((u->sex == 2) ? "♀" : "？"));
  ...
  if (adm != 0)
  {
    ...
    if (find_rform(u->userno, u->userid, address, phone, career))
    {
      ...
      prints("  \033[32m服務單位：\033[37m%-35s\033[m\n", career);
    }
#endif
  }
  else
  {
    ...
    prints("  \033[32m本次停留：\033[37m%d 小時 %d 分\n", diff / 60, diff % 60);
  }
...
void
acct_changeRealName(u)
  ACCT *u;
{
  ACCT x;
  char *str, buf[80];

  /* guessi.100802 擁有帳號相關權限才能操作 */
  if (HAS_PERM(PERM_ALLACCT))
  {
    move(3, 0);
    acct_show(u, 1); /* guessi.100802 必為管理者操作 */

    if (vans("請確認是否執行真實姓名變更 Y)繼續 N)離開 [N] ") != 'y')
      return;
    ...
    do
    {
      vget(4, 0, "真實姓名：", str, RNLEN + 1, GCARRY);
    } while (strlen(str) < 4);

    if (!memcmp(&x, u, sizeof(ACCT)) || vans(msg_sure_ny) != 'y')
    {
      /* guessi.100401 離開前清除畫面 */
      clear();
      return;
    }

    /* guessi.100802 紀錄變更前後資料 */
    sprintf(buf, "使用者[%s] %s -> %s", u->userid, u->realname, x.realname);
    alog("姓名變更", buf);
  ...
  int adm;
{
  ...
  if (adm == 1)
  {
    clrtobot();
    move(12, 0);
    prints("請輸入查詢[\033[1;32m%s\033[m]資料理由，或按[Enter]跳出", u->userid);
    
    if (vget(14, 0, "請輸入理由：", reason, 64, DOECHO))
    {
      move(3, 0);
      acct_show(u, adm);
      sprintf(buf, "使用者[%s] 理由: %s", u->userid, reason);
      alog("查詢帳號", buf);  /* guessi. 060708 紀錄站務行為 */
    }
    else
    {
      move(3, 0);
...
*/
  ...
  for (;;)
  {
    vget(i, 0, "性別 (1)男性 (2)女性：", buf, 2, GCARRY);
  ...
  int row;
{
  ...
  for (;;)
  {
    ...
    if (!strcmp(buf, brdname))    /* Thor: 與舊板名相同則跳過 */
      break;
  ...
  i += 4;
  move(i - 2, 0);
  prints("目前板主為 %s ", brd->BM);

  strcpy(buf, brd->BM);
  BMlen = strlen(buf);

  while (vget(i, 0, "請輸入ID增減板主，結束請按 Enter，清掉所有板主請打「無」： ", userid, IDLEN + 1, DOECHO))
  {
    if (!strcmp(userid, "無"))
    {
      buf[0] = '\0';
  ...
  case 'd':
    ...
    else
    {
      ...
      if (*bname)    /* itoc.000512: 同時砍除同一個看板會造成精華區、看板全毀 */
      {
        ...
        vget(b_lines, 0, "請輸刪除看板原因：", reason, 50, DOECHO);
        sprintf(rbuf, "[%s] 原因: %s", bname, reason);
    ...
    if (!brd_set(&newbh, 11))
    {
      ...
      if (memcmp(&newbh, bhdr, sizeof(BRD)) && vans(msg_sure_ny) == 'y')
      {
        ...
        if (strcmp(bname, newbh.brdname))    /* 看板更名要移目錄 */
        {
          ...
          alog("板名異動", bname); /* guessi.060411 若有更動看板名稱 記錄 */
