  ...
  int max;
{
  ...
  move(i, 0);
  clrtoeol();
  outs("\033[34m──────────────────────── \033[1;37;44m Ctrl + R / T  切換上下 \033[;34m ──\033[m");
}
#endif
  ...
  BMW *bmw;
{
  ...
  {
    prints("%6d%c\033[33m%02d:%02d 送出 %-13s %-*.*s\033[m\n",
      num, tag_char(bmw->btime), ptime->tm_hour, ptime->tm_min, bmw->userid, d_cols + 48, d_cols + 48, bmw->msg);	  
  }
  else					/* 收到的水球 */
  {
    prints("%6d%c%02d:%02d 收到 %-13s %-*.*s\n",
      num, tag_char(bmw->btime), ptime->tm_hour, ptime->tm_min, bmw->userid, d_cols + 48, d_cols + 48, bmw->msg);
  }
}
  ...
  XO *xo;
{
  int fd;
  FILE *fp;
  char buf[64], folder[64], title[128];
  HDR fhdr;
  ACCT acct;
  ...
  if (acct_get(msg_uid, &acct) > 0 && acct.userno != cuser.userno)
  {
	...
	fclose(fp);
      ...
      fhdr.xmode = MAIL_READ | MAIL_NOREPLY;
	  sprintf(title, "[熱訊紀錄] %s", acct.userid);
...
bmw_log()
{
  ...
  if (!stat(fpath, &st) && S_ISREG(st.st_mode))
  {
    ...
    else
    {
      more(buf, (char *) -1);
#ifdef RETAIN_BMW
      op = vans("本次上站水球處理 (M)移至備忘錄 (R)保留 (C)清除 (S)寄給站長存證？[R] ");
#else
