  ...
  HDR *hdr;	/* 條件 */
{
  ...
  /* guessi.120322 xid 0: 不指定作者 xid 1: 指定作者關鍵字 */
  if (hdr->xid && !str_str(head->owner, hdr->owner))
    return 0;
...
static int                      /* 0:不滿足條件  !=0:滿足條件 */
filter_digest(head, hdr)
  HDR *head;    /* 待測物 */
  HDR *hdr;     /* 條件 */
{
  return (head->xmode & POST_DIGEST);
}
...
#endif
  strcpy(HintWord, "\033[1;33m文摘模式\033[m");
  HintAuthor[0] = '\0';

  return XoXpost(xo, NULL, 0, INT_MAX, filter_digest);
}
  ...
  HDR *hdr;
{
  return ((head->xmode & POST_SCORE) &&
          (((hdr->xid == 0) && (head->score < hdr->score)) ||
           ((hdr->xid == 1) && (head->score > hdr->score))));
}
...
#endif
  strcpy(HintWord, "\033[1;33m評分搜尋模式\033[m");
  HintAuthor[0] = '\0';

  if (!vget(b_lines, 0, "搜尋評分 1)高於 2)低於 ", score, 2, LCECHO))
    return XO_FOOT;

  hdr.xid = ((score[0] - '0') == 2) ? 0 : 1;

  if (!vget(b_lines, 0, "評分臨界值 ", score, 4, LCECHO))
    return XO_FOOT;

  hdr.score = (atoi(score) < -99 || atoi(score) > 99) ? 0 : atoi(score);
  ...
  XO *xo;
{
  ...
  for (;;)
  {
    ...
    post_history(xo, hdr); // xpost_history(xo, hdr);
