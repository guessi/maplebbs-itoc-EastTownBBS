  ...
  ACCT *acct;
{
  ...
  *userbm = '\0';
  ...
  hidden = (hidden & UFO_HIDFROM) && !HAS_PERM(PERM_ALLADMIN);

  prints(MSG_SEPERATOR"\n");
  prints("[帳號] %-13s [暱稱] %-18.18s [動態] %-12s\n",
  userid, acct->username,
  (up && can_see(cutmp, up)) ? bmode(up, 1) : "不在站上");

  prints("[上站] %-5d         [文章] %-5d              [信箱] %s新郵件 [認證] %s過認證\n",
  acct->numlogins, acct->numposts, (m_query(userid) & STATUS_BIFF) ? "有" : "無",
  acct->userlevel & PERM_VALID ? "已" : "未");


  if (!hidden)
  {
    if (acct->userlevel) /* 除了GUEST其餘都顯示上次來源 以及上次登入時間*/
      prints("[來源] %-39s [最近] %s \n", acct->lasthost, Btime(&acct->lastlogin));
  } /* UFO_HIDFROM */

  /* guessi.090611 查詢user擔任板主 */
  check_user_bm(userid, userbm);
  if (strlen(userbm) > 0)
    prints("[擔任板主] %s \n", userbm);
