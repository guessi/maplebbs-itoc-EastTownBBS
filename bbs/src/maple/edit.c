...
tbf_ask()
{
  ...
  do {
    ch = vget(b_lines, 0, "請選擇暫存檔(1-3): ", fn_tbf + 4, 2, GCARRY);
  } while(ch < '1' || ch > '3');
...
//  static char msg[] = "選擇簽名檔 (1/2/3 0=不加 r=亂數)[0]：";
  static char msg[] = "顯示簽名檔 (1 ~ 6 0=不加 r=亂數 n=換頁)[0]: ";
...
#ifdef HAVE_ANONYMOUS
  if (!(currbattr & BRD_ANONYMOUS) && cuser.ufo & UFO_SHOWSIGN)	/* 在匿名板中無論是否匿名，均不使用簽名檔 */
#else
  if (cuser.ufo & UFO_SHOWSIGN)					/* itoc.000320: 不使用簽名檔 */
#endif /* 統一改用SHOWSIGN 節省UFO使用 */
  {   
   int topsig = 1;
   show_sign:
...
#ifdef HAVE_ANONYMOUS
      (curredit & EDIT_ANONYMOUS) ? " 一個神祕的地方 " : 
#endif
      (!strcmp(cuser.userid, str_sysop)) ? "bbs.ndhu.edu.tw" : fromhost);
  } /* guessi.060623 若使用帳號為sysop 則ip紀錄為 bbs.ndhu.edu.tw */
  else
  {
    if (strcmp(cuser.userid, str_sysop)) /* 若帳號為sysop則不紀錄修改 */
    {
      fprintf(fp, MODIFY_BANNER,
      cuser.userid, 35 - strlen(cuser.userid), " ", ipv4addr, /*fromhost,*/
...
ve_key:
	...
	break;
      ...
      case Ctrl('Q'):
	cc = vans("插入控制碼 1)ID 2)暱稱 3)時間 4)文章 5)人數 6)生日 7)上站 ");
	if (cc >= '1' && cc <= '7')
	{
	  ve_char(KEY_ESC);
	  ve_char('*');
	  ve_char("sntpubl"[cc - '1']);
	}
	ve_mode = mode | VE_FOOTER;
	break;
#endif
